...
class Block(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.adapter = nn.Linear(cfg.n_embd, cfg.n_embd) # TODO add adapter
    ...
    def forward(self, x, encoder_x):
        ...
        x_cross_adapter = x_cross + F.gelu(self.adapter(x_cross)) # TODO adapter
        x_mlp = x_cross_adapter + self.mlp(self.ln_3(x_cross_adapter))
        return x_mlp

class Block_no_adapter(nn.Module):
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # TODO adapter: 會爆參數，所以有些block不加adapter
        blocks = [Block(cfg) for _ in range(cfg.n_layer - 3)]
        blocks.insert(0, Block_no_adapter(cfg))
        blocks.insert(0, Block_no_adapter(cfg))
        blocks.append(Block_no_adapter(cfg))
